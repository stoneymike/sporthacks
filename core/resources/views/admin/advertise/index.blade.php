                    ...
                    <div class="table-responsive--md  table-responsive">
                        <table class="table table--light style--two">
                            ...
                            @forelse($advertisements as $advr)
                                <tr>
                                    <td data-label="@lang('Advertisement Type')"><span
                                            class="text--small badge font-weight-normal {{$advr->type == 1 ? 'badge--success':'badge--primary'}}">{{$advr->type == 1 ? 'Banner':'Script'}}</span>
                                    </td>
                                    <td data-label=">@lang('Resolution')"><span
                                            class="text--small badge font-weight-normal badge--primary">{{$advr->resolution}}</span>
                                    </td>
                                    @if ($advr->type == 1)
                                        <td data-label="@lang('Ad Image')"><a class="btn btn-sm btn--dark"
                                                                              href="{{getImage('assets/images/advertisement/'.$advr->ad_image)}}"
                                                                              data-rel="lightcase"> <i
                                                    class="las la-eye"></i> @lang('see')</a></td>
                                    @else
                                    ...
                                    <td data-label="@lang('Status')"><span
                                            class="text--small badge font-weight-normal {{$advr->status == 1 ? 'badge--success':'badge--warning'}}">{{$advr->status == 1 ? 'active':'inactive'}}</span>
                                    </td>

                                    <td data-label="Action">
                                        <a href="javascript:void(0)"
                                           class="icon-btn {{ $advr->status ? 'btn--danger' : 'btn--success' }} ml-1 statusBtn"
                                           data-original-title="@lang('Status')" data-toggle="tooltip"
                                           data-url="{{ route('admin.advertise.status', $advr->id) }}">
                                            <i class="la la-eye{{ $advr->status ? '-slash' : null }}"></i>
                                        </a>
                                        <a href="javascript:void(0)" data-advr="{{$advr}}"
                                           data-route="{{route('admin.advertise.update',$advr->id)}}"
                                           class="icon-btn mr-2 ml-2 edit" data-toggle="tooltip" title="@lang('Edit')">
                                            <i class="las la-pen text--shadow"></i>
                                        </a>
                                        <a href="javascript:void(0)"
                                           data-route="{{route('admin.advertise.delete',$advr->id)}}"
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="size" required>
                                    ...
                                    <option value="980x90">@lang('980x90')</option>
                                    <option value="980x120">@lang('980x120')</option>
                            ...
                            <div class="form-group ru">
                                ...
                                <input type="text" class="form-control" name="redirect_url"
                                       placeholder="@lang('https://example.com')" required
                            ...
                            <div class="form-group adfile">
                                ...
                                <div class="file-upload-wrapper" data-text="@lang('Select Your File')">
                                    <input type="file" name="adimage" id="img" class="file-upload-field" required>
                                </div>
                            </div>

                            <div class="form-group script d-none">
                                <label>@lang('Ad Script')</label>
                                <textarea type="text" class="form-control" disabled name="script"
                                          required>{{old('script')}}</textarea>
                            </div>
                        </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group ru">
                                <label>@lang('Redirect Url')</label>
                                <input type="text" class="form-control" name="redirect_url"
                                       placeholder="@lang('http/https://example.com')" required
                                       value="{{old('redirect_url')}}">
                            </div>
...
@push('breadcrumb-plugins')
    ...
    <button type="button" data-toggle="modal" data-target="#adModal" class="btn btn--primary mr-3 mt-2"><i
            class="la la-plus"></i>@lang('Ad New')</button>

    <form action="" method="GET" class="form-inline float-sm-right bg--white mt-2">
        <div class="input-group has_append">
            <input type="text" name="search" class="form-control" placeholder="@lang('Search by ad size')"
...
@push('style-lib')
    <link rel="stylesheet" href="{{asset('assets/admin/css/vendor/lightcase.min.css')}}">
@endpush

@push('script-lib')
    <script src="{{asset('assets/admin/js/vendor/lightcase.min.js')}}"></script>
@endpush
